...
| 06 | [Prefill I/O thread pool](explorations/06-io-threads-prefill/NOTES.md) | Superseded | Implemented through 13 | Prefill: not measured<br>Decode: neutral (13) | The proposed **4→8-worker pool** landed through 13 (`5af6a43`, configurable up to 16). Its decode check was **17.42→17.31 tok/s (neutral)**; decode uses a different dispatch path. **Isolated prefill benefit was never measured.** The prefill side is no longer wholly unmeasured: `67bec6b` (2026-07-08) records prefill **I/O at ~12% of prefill time** and drops pread double-buffering by measurement. `f2ed109` (2026-03-17) separately reverted `F_RDADVISE` kernel read hints as no improvement over plain pread. Both are the implementing commits' own findings, not bench A/Bs. | Closed as a separate proposal; implementation and residual measurement gap belong to 13. Do not put 06 back into the untested queue. |
...
| 09 | [Predictive expert prefetch](explorations/09-expert-prefetch/NOTES.md) | Parked | Negative feasibility probe | — Speed not measured; coverage probe only | With pinning off, previous-token experts covered **32.8%** of reads; with pinning on, only **5.7% (4 GiB)** / **8.2% (3 GiB, pressure emulation)** of remaining misses were predictable. This measured predictor coverage, **not a prefetch throughput gain/loss**; no prefetch engine was built. An earlier prefetch engine does exist outside this campaign: `f2ed109` (2026-03-17) built speculative early routing, reached **53% prediction accuracy** on the pre-attention hidden state, and still measured **38% slower (1.70 vs 2.75 tok/s)** because predictions evicted useful cache entries and added `gate_proj` CPU and dispatch overhead. It was disabled, not deleted. That is the commit's own measurement. | Reject the tested cheap predictors as planned; hot repeats were already pinned. Learned routing remains parked pending a concrete predictor, miss coverage, and target-hardware evidence. Old ~10% upside was an estimate. Treat prediction accuracy alone as insufficient: the one built prefetcher was accurate and still net negative through cache pollution. |
...
| 28 | [Cache prepared prompts, tokens, and schemas](explorations/28-prepared-prompt-cache/NOTES.md) | Closed | Preparation gain measured; end-to-end effect inconclusive | **~3.9 ms saved per 26 KiB tool-prefix hit**; no demonstrated end-to-end gain | 20 paired CPU comparisons per case: 24-tool hits **3.912→0.035 ms**, misses **4.129→1.996 ms**; every output check passed. The standard q4/q8 comparison was not isolated enough to attribute its mixed changes to this cache. | Keep the bounded one-entry cache as a modest CPU preparation optimization. Schema normalization remains deferred because it would change rendered prompt bytes. |

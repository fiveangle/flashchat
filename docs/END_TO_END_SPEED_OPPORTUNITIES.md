...
| 26 | Recover exact conversation-state reuse | Untested | No measurement | — Not measured | No new measurement. Old session reuse existed (`d965514`, `d5e5e71`) and was removed in `bbaa549`; current API restores only system/tool state. Proposed exact matching/checkpoints are not implemented. | Define exact token matching and valid hybrid-state checkpoints. |

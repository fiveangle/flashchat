#!/usr/bin/env python3
"""Extract all non-expert weights from a model's safetensors into a single binary blob.

Reads model dimensions from assets/model_configs.json. Supports 4-bit and 8-bit
quantization (auto-converts 8-bit overrides to 4-bit when present in config.json).
...
def load_registry():
    path = os.environ.get("FLASHCHAT_MODEL_CONFIG", REPO_ROOT / "assets" / "model_configs.json")
    with open(path) as f:
        return json.load(f)
...
def get_hf_repo(model_id):
    return get_model_entry(model_id)["hf_repo"]
...
def main():
    ...
    model_path = Path(args.model) if args.model else Path(detect_model_path(entry["hf_repo"]))
    if args.output:

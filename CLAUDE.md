...
- `metal_infer/infer.m` is a single ~15,000+ line Objective-C/C file. Never `Read` it untargeted — first locate line numbers with `grep -n` or `awk 'NR>=X && NR<=Y'`, then `Read` with `offset`/`limit` or `sed -n 'X,Yp'` (quoted) on just that range.
- `metal_infer/shaders.metal` holds the Metal compute kernels (matmul/dequant/attention); kernel variant names follow `*_v3`/`*_v4`/`*_2` (batched) suffixes.

### Architecture Facts
*~4,000 tokens/session saved*
- Dense models mmap the **entire** weight file into RAM (no expert streaming); MoE models stream/page experts from disk per-token. Running a freshly-built `infer` against a large model without checking free RAM vs. model size has OOM-crashed the host machine (required hard restart) — this happened with both Codex and Claude.
- Active-expert count `K` and similar per-model tunables (e.g. batched-prefill/MTP eligibility) are read from the model registry/recipe JSON (`assets/model_configs.json`), not hardcoded — they vary per model (e.g. K=8 for 35B-A3B, K=10 for 397B).

### Project Structure
*~3,000 tokens/session saved*
- Python model-management framework lives in `modelmgr/` (was `flashchat_models/`, before that ad-hoc `scripts/*.py` — the legacy `scripts/` dir was removed as redundant). Use `python3 -m modelmgr <cmd>`.
- The Apple Neural Engine (ANE) dense-MLP acceleration path was fully removed as a failed experiment — don't search for or reintroduce `ane_dense`/`private_ane`/`hasANE` code; it's git history only.
- Use `metal_infer/.venv/bin/python` (not system `python3`) for scripts needing `numpy`/`safetensors` — project policy is no system Python installs.

### Environment
*~2,500 tokens/session saved*
- The project's `grep` is aliased to `ugrep`, which has stricter regex parsing than GNU grep — escaped parens `\(`/`\)` in alternation patterns often throw `mismatched ( )` syntax errors. Prefer unescaped literal parens or `grep -F` for fixed strings.
- In zsh, unquoted comma-separated `sed -n` ranges (e.g. `sed -n 2955,2975p`) fail with exit code 1 — always quote the range: `sed -n '2955,2975p'`.

### Build & Test
*~1,500 tokens/session saved*
- Standard build/verify loop: `make infer 2>&1 | grep -iE "error|warning:"` (or `make` for all targets), then run the binary directly or via `./flashchat serve`.
- Test suites: `make test` (full), `make py-tests` (Python unit tests under `tests/python/`), or individual `tests/test_*.sh` / `tests/bench_*.sh` scripts.

### Model Storage Locations
*~1,500 tokens/session saved*
- Fast model artifacts live under `/Volumes/usr/Users/speedster/dev/models/hf` (Samsung 970 Pro SSD, ~1GB/s) — prefer this for performance testing.
- `/Volumes/dev/flashchat/models-backup` is a slower spinning-disk backup copy of older model snapshots (~140MB/s) — only use when the SSD copy is unavailable.

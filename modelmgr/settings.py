...
class Setting:
    ...
    inverted: bool = False     # toggle on stores 0 instead of 1
...
ADVANCED = (
    Setting("IO_THREADS", "advanced", "Parallel expert disk readers",
            ...
            help="Number of workers reading expert weights from disk at once. More workers can help keep the GPU supplied, but too many can increase disk contention."),
    Setting("GPU_ROPE", "advanced", "GPU rotary position encoding",
            "GPU rotary position encoding (0/1)", kind="bool",
            help="Applies token-position information on the GPU instead of the CPU. Normally leave enabled; disabling is useful for diagnosing the GPU path."),
    ...
    Setting("PREFILL_RELEASE", "advanced", "Release temporary prompt-processing memory",
            ...
            help="Releases temporary buffers after reading the prompt to leave more RAM available during generation. Later prompts may need to allocate them again."),
    Setting("SERVER_DEBUG", "advanced", "Server debug logging",
            "Server debug logging to server.log (0/1)", kind="bool",
            help="Adds diagnostic details to the server log. Useful for troubleshooting; increases log volume."),
    Setting("SERVER_HTTP_LOG", "advanced", "HTTP request/response log",
            "HTTP request/response log to http.log (0/1)", kind="bool",
            help="Records API requests and responses in http.log for troubleshooting clients and tool calls. Logs can contain prompt and response content."),
    Setting("BATCH_PREFILL", "advanced", "Batched prompt processing",
            "Batched prompt processing (0/1)",
            "Processes multiple prompt tokens together to improve prompt-reading speed. Larger batches use more temporary RAM.", kind="bool"),
    ...
    Setting("FUSE_LINEAR", "advanced", "Fused GPU scheduling for linear-attention layers",
            ...
            "Combines GPU work submissions for linear-attention layers to reduce scheduling overhead. Normally leave enabled; disabling helps compare or diagnose this path.",
            kind="bool"),
    Setting("ADAPTIVE_K_MASS", "advanced", "Adaptive expert count by routing mass",
            ...
            "Uses fewer experts when routing is concentrated, reducing disk reads. Lower values skip more work but can change generated output. Leave empty to disable.",
    ...
    Setting("PREFILL_DEBUG", "advanced", "Prompt-processing debug",
            ...
            help="Chunk timings report time spent reading each prompt chunk. State dumps add internal model values and can slow processing substantially. Leave off for normal use.",
    ...
    Setting("PREAD_PROFILE_CAP", "advanced", "Max recorded disk-read events",
            ...
            help="Stops recording disk-read timing events after this limit. Limits profiling data volume; inference continues normally.",
    ...
    Setting("EXPERT_PIN_MAX_EXPERTS", "advanced", "Expert RAM cache target (experts)",
            ...
            help="Maximum number of complete experts to keep in RAM. Leave empty to size the cache from its memory limits. The GiB and free-RAM limits still apply.",
    ...
    Setting("EXPERT_PIN_MAX_GB", "advanced", "Expert cache limit (GiB)",
            ...
            help="Maximum RAM used to retain expert weights and avoid repeated disk reads. Set to 0 to disable the cache. Available RAM may reduce the actual allocation.",
    ...
    Setting("EXPERT_PIN_AUTO_FRAC", "advanced", "Expert cache share of free RAM",
            ...
            help="Further limits the expert cache to this fraction of free RAM when initialized. For example, 0.5 allows half the free RAM, subject to the GiB limit.",
    ...
    Setting("SYSTEM_PROMPT_CACHE_MAX_ENTRIES", "advanced", "Max saved system prompts",
            ...
            help="Maximum number of persistent system-prompt cache entries. More entries reuse more distinct system prompts but consume more disk space; entries can be tens of MB.",
    ...
    Setting("SYSTEM_PROMPT_CACHE_DIR", "advanced", "System prompt cache folder",
            ...
            help="Folder for persistent system-prompt caches. Leave empty to store them with the model's runtime files. Reusing a saved prompt avoids processing it again.",
    ...
    Setting("MTP", "advanced", "Multi-token prediction",
            ...
            "Drafts upcoming tokens with a predictor head, then checks them with the main model before accepting them. Requires a model with predictor artifacts. Larger draft batches can help when predictions are often accepted, but waste work when they are rejected.",
            kind="mtp", empty_title="Model default"),
    Setting("MTP_BF16", "advanced", "Use reduced-precision predictor weights",
            "  ^ use reduced-precision predictor weights (0/1)",
            "Uses the model’s bundled quantized predictor (q4 for 4-bit models, q8 for 8-bit models) to reduce memory use and potentially speed up drafting. Reduced precision may lower the proportion of predictions accepted by the main model, offsetting that speed benefit. Predictions are still verified by the main model. Turn off to request higher-precision 16-bit (BF16) predictor weights; if unavailable, the engine falls back to the model's bundled predictor. Only applies when multi-token prediction is active.",
            kind="bool", parent="MTP", inverted=True),
    Setting("SHOW_THINKING", "advanced", "Show thinking tokens",
            "Show thinking tokens (0/1)", kind="bool",
            help="Shows the model's thinking text in terminal chat. Controls display; use Reasoning mode to change whether the model generates thinking text."),
    Setting("COLOR_OUTPUT", "advanced", "Color output", "Color output (0/1)", kind="bool",
            help="Uses color in terminal output. Does not change the menubar app's appearance."),
)
